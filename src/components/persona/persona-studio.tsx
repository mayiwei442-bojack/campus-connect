...
import {
  MAX_PERSONA_IMAGE_ANALYSIS_BYTES,
...
export function PersonaStudio({ isOwner, personas, showLocalRoadshow, viewerId }: PersonaStudioProps) {
  ...
  async function uploadAsset(event: FormEvent<HTMLFormElement>, personaId: string) {
    ...
    try {
      ...
      if (file.size > MAX_PERSONA_IMAGE_ANALYSIS_BYTES) {
        finish("图片原图已保存；因超过 7 MB，已跳过自动理解，可继续人工录入。");
        return;
      }
