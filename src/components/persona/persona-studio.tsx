...
export function PersonaStudio({ isOwner, personas, viewerId }: { isOwner: boolean; personas: PersonaItem[]; viewerId: string }) {
  ...
  async function uploadAsset(event: FormEvent<HTMLFormElement>, personaId: string) {
    ...
    const validationError = validatePersonaImage(file);
    if (validationError) return setError(validationError);

    begin(`upload:${personaId}`);
    const description = String(data.get("description") ?? "").trim();
    let orphanPath = `${viewerId}/${personaId}/${crypto.randomUUID()}.${personaImageExtension(file.type)}`;

    try {
      const { error: uploadError } = await supabase.storage.from("persona-assets").upload(orphanPath, file, {
        cacheControl: "3600",
        contentType: file.type,
        upsert: false,
      });
      if (uploadError) throw new Error("图片上传失败，请检查网络或存储空间后重试。");
      ...
      const payload = await response.json() as { compressed?: boolean; error?: string; entryCount?: number };
      if (!response.ok) {
        finish();
        setError(`图片已安全保存；${payload.error || "自动理解暂时不可用，可重试或人工录入。"}`);
        return;
      }
      finish(payload.compressed
        ? `原图已保存，并已自动压缩后分析，生成 ${payload.entryCount ?? 0} 条待确认草稿。`
        : `图片已保存，生成 ${payload.entryCount ?? 0} 条待确认草稿。`);
    } catch (cause) {
      if (orphanPath) await supabase.storage.from("persona-assets").remove([orphanPath]);
      fail(cause, "图片上传失败。");
    }
  }
  ...
  async function analyzeAsset(personaId: string, assetId: string) {
    ...
    finish(payload.compressed
      ? `已自动压缩图片，并生成 ${payload.entryCount ?? 0} 条新的待确认草稿。`
      : `已生成 ${payload.entryCount ?? 0} 条新的待确认草稿。`);
  }
