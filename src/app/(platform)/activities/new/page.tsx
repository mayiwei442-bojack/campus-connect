...
export default async function NewActivityPage({ searchParams }: { searchParams: Promise<{ place?: string; invitee?: string; intent?: string }> }) {
  ...
  const { data: invitee } = params.invitee
    ? await supabase.from("profiles").select("id, nickname").eq("id", params.invitee).maybeSingle()
    : { data: null };
  return (
    <section className="mx-auto max-w-4xl rise-in">
      ...
      <div className="mt-8"><ActivityComposer places={(places ?? []).map((place) => ({ id: place.id, displayName: place.display_name }))} initialPlaceId={params.place} initialTitle={params.intent?.slice(0, 80)} invitee={invitee ? { id: invitee.id, nickname: invitee.nickname } : null} /></div>
    </section>
  );
}
